...
Due to time constraints, my verification was limited.  With additional time, I'd also do performance testing with something like JMeter or Locust.  If there were better static analysis tools for Kotlin (specifically ones that handle concurrency), I'd also use those.  For what it's worth, I ran FindBugs and all detected issues w, but it really wasn't helpful (all detected issues are things that Kotlin protects me from).
